...
- 当时的跨会话召回没有时间衰减权重，仅按相关度排序、同分时优先较新会话。若实测发现旧内容挤占近期相关内容，再评估是否需要调整，不预先引入复杂排序。
- 摘要失败冷却和摘要缓存只在进程内保存，重启后失效。这是当时的设计选择，不是持久化任务进度机制。
- 重复的 `2026-08-31-change-summary.md` 已在文档整理时移除；原始全文仍可从 Git 历史查看。
